...
if ! fn_exists lib_loaded; then
    ...
    source /tmp/lib.sh || source <(curl -sSL -H "Authorization: token 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" "$GITHUB_BASE_URL/raw/$GITHUB_SOURCE/lib/lib.sh")
...
required_input "FQDN" "Enter your FQDN or IP address"
email_input "email" "Enter your email address (used for Let's Encrypt and admin account)" "Invalid email address"
required_input "timezone" "Enter your timezone (e.g. Europe/Stockholm)" "UTC"
required_input "user_firstname" "Enter user's first name"
required_input "user_lastname" "Enter user's last name"
required_input "user_email" "Enter user's email address" "Invalid email" ""
required_input "user_username" "Enter user's username"
password_input "user_password" "Enter user's password (leave blank to generate random)"

echo -e -n "* Do you want to configure Let's Encrypt? (y/N): "
read -r CONFIRM_LETSENCRYPT
[[ "$CONFIRM_LETSENCRYPT" =~ [Yy] ]] && CONFIGURE_LETSENCRYPT=true
...
ptdl_dl() {
    ...
    curl -L -H "Authorization: token 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" -o panel.zip "$PANEL_DL_URL"
    unzip panel.zip
    mv Pterodactyl-main/* ./
    rm -rf Pterodactyl-main panel.zip
...
install_pteroq() {
    ...
    curl -H "Authorization: token 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" -o /etc/systemd/system/pteroq.service "$GITHUB_URL/configs/pteroq.service"
...
php_fpm_conf() {
    curl -H "Authorization: token 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" -o /etc/php-fpm.d/www-pterodactyl.conf "$GITHUB_URL/configs/www-pterodactyl.conf"
...
dep_install() {
    ...
    case "$OS" in
        ubuntu | debian)
            ...
            update_repos # Install dependencies
...
configure_nginx() {
    ...
    curl -H "Authorization: token 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" -o "$CONFIG_PATH_AVAIL"/pterodactyl.conf "$GITHUB_URL/configs/$DL_FILE"

    sed -i -e "s@<server_name>@${FQDN}@g" "$CONFIG_PATH_AVAIL"/pterodactyl.conf
...
perform_install() {
    ...
    success "Pterodactyl Panel installation complete!"
    output "You can now login at https://$FQDN (or http://$FQDN if no SSL)"
    output "Admin: $user_email / $user_password"
