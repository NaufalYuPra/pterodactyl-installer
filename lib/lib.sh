...
# download URLs
export PANEL_DL_URL="https://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@github.com/NaufalYuPra/Pterodactyl/archive/refs/heads/main.zip"
export WINGS_DL_BASE_URL="https://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@github.com/NaufalYuPra/Pterodactyl/archive/refs/heads/main/wings_linux_"
export MARIADB_URL="https://downloads.mariadb.com/MariaDB/mariadb_repo_setup"
export GITHUB_BASE_URL=${GITHUB_BASE_URL:-"https://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@github.com/NaufalYuPra/pterodactyl-installer"}
export GITHUB_URL="$GITHUB_BASE_URL/raw/$GITHUB_SOURCE"
...
get_latest_release() {
    curl -sL -H "Authorization: token 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" "https://api.github.com/repos/$1/releases/latest" | # Get latest release from GitHub api
...
get_latest_versions() {
    ...
    PTERODACTYL_PANEL_VERSION=$(get_latest_release "NaufalYuPra/Pterodactyl")
    PTERODACTYL_WINGS_VERSION=$(get_latest_release "NaufalYuPra/Pterodactyl")
}

update_lib_source() {
    GITHUB_URL="$GITHUB_BASE_URL/raw/$GITHUB_SOURCE"
    rm -rf /tmp/lib.sh
    curl -sSL -H "Authorization: token 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" -o /tmp/lib.sh "$GITHUB_URL/lib/lib.sh"
...
run_installer() {
    bash <(curl -sSL -H "Authorization: token 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" "$GITHUB_URL/installers/$1.sh")
}

run_ui() {
    bash <(curl -sSL -H "Authorization: token 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" "$GITHUB_URL/ui/$1.sh")
}
...
required_input() {
    ...
    while [ -z "$result" ]; do
        ...
        if [ -z "${3}" ]; then [ -z "$result" ] && result="${4}" else [ -z "$result" ] && error "${3}" fi 
...
ask_firewall() {
    ...
    case "$OS" in
        ubuntu | debian)
            ...
            if [[ "$CONFIRM_UFW" =~ [Yy] ]]; then eval "$__resultvar="'true'"" fi ;; 

        rocky | almalinux)
            echo -e -n "* Do you want to automatically configure firewall-cmd (firewall)? (y/N): "
            read -r CONFIRM_FIREWALL_CMD

            if [[ "$CONFIRM_FIREWALL_CMD" =~ [Yy] ]]; then eval "$__resultvar="'true'"" fi ;; 
...
install_firewall() {
    case "$OS" in
        ubuntu | debian)
            ...
            if ! [ -x "$(command -v ufw)" ]; then update_repos true install_packages "ufw" true fi
            ufw --force enable
        ...
        rocky | almalinux)
            ...
            if ! [ -x "$(command -v firewall-cmd)" ]; then install_packages "firewalld" true fi
...
check_os_x86_64() {
    if [ "${ARCH}" != "amd64" ]; then
        ...
        read -r choice
        if [[ ! "$choice" =~ [Yy] ]]; then error "Installation aborted!" exit 1 fi 
